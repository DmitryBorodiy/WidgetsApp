...
namespace HtmlToXamlDemo
{
    ...
    public static class HtmlToXamlConverter
    {
        ...
        private static int _defaultFontSize = 14;
        ...
        /// </returns>
        public static string ConvertHtmlToXaml(string htmlString, bool asFlowDocument, string defaultFontFamily = null, int defaultFontSize = 14)
        {
            ...
            if(!string.IsNullOrEmpty(defaultFontFamily) &&
               xamlFlowDocumentElement.GetAttribute(XamlFontFamily) == string.Empty)
               xamlFlowDocumentElement.SetAttribute(XamlFontFamily, defaultFontFamily);
            ...
            while (propertyEnumerator.MoveNext())
            {
                switch ((string) propertyEnumerator.Key)
                {
                    ...
                    case "font-weight":
                        ...
                        break;
                    case "font-size":
                        string fontSize = (string)propertyEnumerator.Value;
                        
                        if(double.TryParse(fontSize.TrimEnd('%'), NumberStyles.Any, CultureInfo.InvariantCulture, out double percent))
                        {
                            double baseSize = _defaultFontSize;
                            double computedSize = baseSize * percent / 100.0;

                            xamlElement.SetAttribute(XamlFontSize, computedSize.ToString(CultureInfo.InvariantCulture));
                        }
                        else if(fontSize.EndsWith("px"))
                        {
                            if(double.TryParse(fontSize.Replace("px", ""), NumberStyles.Any, CultureInfo.InvariantCulture, out double px))
                               xamlElement.SetAttribute(XamlFontSize, px.ToString(CultureInfo.InvariantCulture));
                        }
                        else if(double.TryParse(fontSize, NumberStyles.Any, CultureInfo.InvariantCulture, out double absoluteSize))
                           xamlElement.SetAttribute(XamlFontSize, absoluteSize.ToString(CultureInfo.InvariantCulture));
                        else
                           xamlElement.SetAttribute(XamlFontSize, _defaultFontSize.ToString(CultureInfo.InvariantCulture));
                        break;
                    case "color":
